...
export const generateChapterContent = async (
  ...
  const systemMessage = `You are an expert educational content creator specialized in creating comprehensive, engaging educational content.
Format your response with clear headings, paragraphs, and proper markdown for formulas and code blocks.
${preferences ? `Consider these user preferences:
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Create detailed educational content for chapter "${chapterTitle}" (${chapterDescription}).
...
Use #, ## for headings, **bold**, *italic*, code fences, and LaTeX for math.`
          }
        ],
        temperature: 0.7,
        max_tokens: 4000
...
export const generateFlashcards = async (
  ...
  const preferences = await getUserPreferences(userId);
  const systemMessage = `You are an expert flashcard creator. Respond with ONLY a JSON array of ${count} objects:
[ { "question": "...", "answer": "..." }, ... ]
No extra text or markdown.
${preferences ? `Level: ${preferences.education_level}, Age: ${preferences.age}, Experience: ${preferences.experience_level}` : ''}`;
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Generate ${count} flashcards from the following chapter content:
"""
${chapterContent}
"""
...
export const generateTestQuestions = async (
  ...
  const preferences = await getUserPreferences(userId);
  const systemMessage = `You are an expert test question creator. Respond with ONLY a JSON array of ${count} objects:
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Generate ${count} multiple-choice questions from the chapter content:
"""
${chapterContent}
"""
Each must have question, 4 options, and correctAnswer index.`
          }
        ],
        temperature: 0.7,
        max_tokens: 2000
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You are an expert educational assistant. Provide concise explanations why an answer is correct or incorrect." },
          {
            role: "user",
